...
// Callback function to update the robot's pose from OpenVINS
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    current_pose = *msg; // Store the current pose
}

int main(int argc, char** argv)
{
    // Initialize ROS
    ros::init(argc, argv, "husky_openvins");
    ros::NodeHandle nh;
    ...
    // Subscribe to the OpenVINS pose topic (modify topic name as needed)
    ros::Subscriber pose_sub = nh.subscribe("/ov_msckf/poseimu", 10, poseCallback);
